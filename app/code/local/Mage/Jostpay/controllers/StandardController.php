...
class Mage_Jostpay_StandardController extends Mage_Core_Controller_Front_Action
{
    public $isValidResponse = false;

    /**
     * Get singleton with Jostpay strandard
     *

     */
    public function getStandard()
    {
        return Mage::getSingleton('jostpay/standard');
    }
	...
	function getTransaction($transaction_reference){
		...
		$readresult = $read->query("SELECT * FROM mage_jostpay WHERE transaction_reference='$transaction_reference' LIMIT 1");
    ...
    /**
     * When a customer chooses Jostpay on Checkout/Payment page
     *
     */
    public function redirectAction()
    {
        $session = Mage::getSingleton('checkout/session');
        $session->setJostpayStandardQuoteId($session->getQuoteId());

        $order = Mage::getModel('sales/order');
        $order->loadByIncrementId($session->getLastRealOrderId());
        $order->addStatusToHistory(
            $order->getStatus(),
            Mage::helper('jostpay')->__('Customer was redirected to Jostpay')
        );
        $order->save();

        $this->getResponse()
            ->setBody($this->getLayout()
                ->createBlock('jostpay/standard_redirect')
                ->setOrder($order)
                ->toHtml());
    ...
    public function  successResponseAction()
    {
		...
		/*
		if ($this->getDebug()) {
			Mage::getModel('jostpay/api_debug')
		...
		*/
		...
		else 
		{
			...
			{
				$jostpay_merchant_id=$this->getConfig()->getMerchantID();
				...
				$url="https://jostpay.com/api_v1?jostpay_id=$jostpay_merchant_id&action=get_transaction&ref=$transaction_reference&amount=$order_amount";
				...
				if(!empty($json))
				{
					...
					$order = Mage::getModel('sales/order');
						...
						if (!$order->getId()) {
							/*
							* need to have logic when there is no order with the order id from Jostpay
							*/
							return false;
						}
						...
						$order->addStatusToHistory(
							$order->getStatus(),
							Mage::helper('jostpay')->__('Customer successfully returned from Jostpay')
						);
						...
						if ($new_status==-1) {
							...
							$order->addStatusToHistory(
								$order->getStatus(),
								Mage::helper('jostpay')->__($response_description)
							);
							...
							$order->addStatusToHistory(
								$order->getStatus(),
								Mage::helper('jostpay')->__('Order total amount does not match Jostpay gross total amount')
							);
						} else {
						...
						$session = Mage::getSingleton('checkout/session');
						$session->setQuoteId($session->getJostpayStandardQuoteId(true));
    ...
    public function historyAction () //aka failureAction obsolete
    {
		...
		*/
        ...
        $this->loadLayout();
        $this->_initLayoutMessages('jostpay/session');
        $this->renderLayout();
    }
	
	protected function getHistoryURL ()
    {
        return Mage::getUrl('jostpay/standard/successresponse');
    }
	...
	public function renderResponse ($trans)
	{
		...
		else 
		{
			...
			$sql="SELECT COUNT(*) as count_all FROM mage_jostpay $dwhere";
			$read = Mage::getSingleton('core/resource')->getConnection('core_read');
			...
			if($num==0)$toecho.="<strong>No record found from transactions made through Jostpay</strong>";
			else
			{
				$perpage=10;
				...
				if($p<1)$p=1;
				$offset=($p-1) *$perpage;
				$sql="SELECT * FROM mage_jostpay $dwhere ORDER BY id DESC LIMIT $offset,$perpage ";
		...
		<head>
			...
			<title>Jostpay Payment Transaction</title>
